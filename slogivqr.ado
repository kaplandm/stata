*! version 0.0.1  25jan2025  kaplandm.github.io
program slogivqr, eclass properties(svyb) byable(recall)
 version 11
 if (!replay()) {
  ...
  * Parse arguments for slogivqr
  ...
  qui su ``lhs''
  if (r(min)<0) {
    di as error "{bf:depvar} must have all non-negative values"
    exit 198
  }
  else if (`quantile'<=0) {
  ...
  * Main subroutine call
  tempname slogivqrb slogivqrV slogivqrh slogivqrhhat slogivqrhhatmax initname
  if ("`initial'"=="") {
    tempvar loglhs
    qui gen `loglhs' = log(``lhs'')
    * qreg doesn't support noconstant, so adjust after
    if ("`weight'"!="") {
      * Using aweight so can use Stata version 11; o/w need Stata 12 for pw/iw
      qui qreg `loglhs' ``Xendo'' ``Xexog'' if `touse' [aw=`exp'] , quantile(`quantile')
    }
    else {
      qui qreg `loglhs' ``Xendo'' ``Xexog'' if `touse' , quantile(`quantile')
    }
    matrix `initname' = e(b)
  ...
  mata: slogivqrmain("``lhs''", "``Xendo''", "``Xexog''", "``Zexcl''", "`touse'", `quantile', `bandwidth', `reps', "`logiterations'"=="logiterations", "`constant'"=="noconstant", "`wgtvar'", `seed', "`dots'"=="nodots", "`slogivqrb'", "`slogivqrV'", "`slogivqrh'", "`slogivqrhhat'", "`slogivqrhhatmax'", "`initname'")
  set seed ``seedname''

  * Store return values
  if ("`constant'"=="noconstant") {
    matrix colnames `slogivqrb' = ``Xendo'' ``Xexog''
  }
  else {
    matrix colnames `slogivqrb' = ``Xendo'' ``Xexog'' _cons
  }
  ...
  capture confirm matrix `slogivqrV'
  // [redundant w/ ereturn post]  ereturn clear
  if _rc { // `slogivqrV' does not exist
    ereturn post `slogivqrb' ``erpwgt'', ``erpopt'' properties("b")
  }
  else { // `slogivqrV' exists
    local names : colfullnames `slogivqrb'
    matrix rownames `slogivqrV' = `names'
    matrix colnames `slogivqrV' = `names'
    ereturn post `slogivqrb' `slogivqrV' ``erpwgt'', ``erpopt'' properties("b V")
  }
  ...
  ereturn local title "Smoothed instrumental variables quantile regression (slogivqr)"
  if ("`constant'"=="noconstant") {
    ereturn local constant "noconstant"
  }
  ereturn scalar bwidth = `slogivqrh' //matches qreg name (though different meaning!)
  ereturn scalar bwidth_req = `slogivqrhhat'
  if (`bandwidth'<0) {
    ereturn scalar bwidth_max = `slogivqrhhatmax'
  }
  ...
  ereturn local cmd "slogivqr" // should be last to store (according to ereturn entry in Stata Manual)
 }
 else { // replay
  if `"`e(cmd)'"' != "slogivqr" { 
    error 301
  ...
  * Print results header; see https://www.stata.com/manuals13/pdisplay.pdf
  di as text ""
  di as text "Smoothed log-linear IV quantile regression (slogivqr)" ///
...
version 11
mata:
void slogivqrmain(string scalar Yname, string matrix Dname, string matrix Xexogname, string matrix Zexclname, string scalar touse, real scalar tau, real scalar h, real scalar reps, real scalar logiterations, real scalar noconst, string scalar wgtname, real scalar seed, real scalar nodots, string scalar slogivqrbname, string scalar slogivqrVname, string scalar slogivqrhname, string scalar slogivqrhhatname, string scalar slogivqrhhatmaxname, string scalar binitname) {
  real colvector Y, binit, weights, slogivqrb, wstar, IQRs, hhats
  real matrix D, Xexog, Zexcl, Z, X, tmp, bstars, corrmat
  real scalar n, hhat, db, slogivqrh, i, junk, k25, k75, eps25, eps75, p25, p75, N01iqr, Vhat, Vsd, sighat, hVCE
  ...
  if (db<cols(X)) {
    _error("The matrix (row vector) named in the initial() option is too short; may be due to perfect multicollinearity (try running qreg or ivregress to see if any regressors are dropped due to perfect multicollinearity, then re-run slogivqr without those variables)")
  }
  else if (db>cols(X)) {
  ...
  if (h<0) {
    ...
    // binit = slogivqrest(tau, Y, X, Z, 0, binit, weights, junk, iterlog)
    hhats = slogivqrbw(tau, Y, X, binit, n, db)
    binit = slogivqrest(tau, Y, X, Z, hhats[1], binit, weights, junk, iterlog)
    hhats = slogivqrbw(tau, Y, X, binit, n, db)
    hhat = hhats[1]
  }
  else {
  ...
  slogivqrh = .
  retraw = slogivqrest(tau, Y, X, Z, hhat, binit, weights, slogivqrh, iterlog)
  slogivqrb = retraw
  st_numscalar(slogivqrhhatname, hhat)
  if (length(hhats)>1) {
    st_numscalar(slogivqrhhatmaxname, max(hhats))
  }
  st_numscalar(slogivqrhname, slogivqrh)
  st_matrix(slogivqrbname, slogivqrb')
  ...
  if (reps>=2) {
    ...
    for (i=1; i<=reps; i++) {
      ...
      bstars[i,.] = slogivqrest(tau, Y, X, Z, slogivqrh, slogivqrb, wstar, junk, iterlog)'
      if (!nodots) {
        printf(".")
        if (mod(i,50)==0) {
          printf("%6.0g", i)
    ...
    if (reps<4) {
      st_matrix( slogivqrVname , variance(bstars) )
    }
    else {
      ...
      st_matrix( slogivqrVname , correlation(bstars):*(IQRs*IQRs')/N01iqr^2 )
    }
  }
  else { // compute analytic SE
    // not possible because would have infinite residuals: Vhat = log(Y) - X*slogivqrb
    printf("Analytic standard errors not available for slogivqr; use reps(#) to run # bootstrap replications.\n")
  }
}


// Try to estimate with bandwidth hhat; increase if necessary to get numerical solution
real colvector slogivqrest(real scalar tau, real colvector Y, real matrix X, real matrix Z, real scalar hhat, real colvector binit, real colvector weights, real scalar storeHhere, string scalar iterlog) {
  ...
  solvenl_init_conv_nearzero(S, 1e-6) // default 1e-9 too strict when many dummies in some cases
  ...
  if (hcur>HMAX) {
    ...
    if (lastec==21) {
      printf("May be due to perfect multicollinearity; try running qreg or ivregress to see if any regressors are dropped due to perfect multicollinearity, then re-run slogivqr without those variables.\n")
    }
...
real colvector slogivqrbw(real scalar tau, real colvector Y, real matrix X, real colvector binit, real scalar n, real scalar db) {
  real scalar k, eps, k25, eps25, k75, eps75, p25, p75
  real scalar h1, h2, h3, Vsd, sighat
  real colvector Vhat

  // Get residuals based on binit
  Vhat = log(Y) - X*binit  // Vhat = Y - X*binit
  Vhat = Vhat[selectindex(Vhat:!=.)]
  ...
  h1 = slogivqrbwK(tau, n, db, sighat, Vhat)

  // Compute Gaussian reference h
  h2 = slogivqrbwG(tau, n, db, sighat)  //adj'd to allow tau=.5
...
real scalar slogivqrbwK(real scalar tau, real scalar n, real scalar db, real scalar sighat, real colvector Vhat) {
...
real scalar slogivqrbwG(real scalar tau, real scalar n, real scalar db, real scalar sighat) {
...
void objnl(real colvector b, real colvector values, real matrix Zw, real matrix X, real colvector htaunY) {
  Ivec = Itilde(-(log(htaunY[|4\.|])-X*b)/htaunY[1])  // J(length(htaunY)-3,1,0)
  inds = selectindex(htaunY[|4\.|]:==0)
  Ivec[inds] = J(length(inds),1,1)
  values = quadcross(Zw , (Ivec - J(htaunY[3],1,htaunY[2]))) / htaunY[3]
}
